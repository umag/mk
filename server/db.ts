...
export function openDb(path: string): DatabaseSync {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS cards   (id TEXT PRIMARY KEY, column_id TEXT, title TEXT, notes TEXT, due TEXT, entered_at INTEGER, pos INTEGER);
...
export function loadAll(db: DatabaseSync): WorldState {
  ...
  const cardStmt = db.prepare("SELECT id,title,notes,due,entered_at FROM cards WHERE column_id=? ORDER BY pos");
  ...
  return {
    boards: boards.map((b): Board => ({
      ...
      columns: (colStmt.all(b.id) as Array<{ id: string; name: string; wip: number | null }>).map((c): Column => ({
        ...
        cards: (cardStmt.all(c.id) as Array<{ id: string; title: string; notes: string; due: string | null; entered_at: number }>).map((k): Card => ({
          id: k.id, title: k.title, notes: k.notes, due: k.due, enteredColumnAt: k.entered_at,
...
export function saveAll(db: DatabaseSync, world: WorldState): void {
  ...
  try {
    ...
    const ik = db.prepare("INSERT INTO cards (id,column_id,title,notes,due,entered_at,pos) VALUES (?,?,?,?,?,?,?)");
    ...
    world.boards.forEach((b, bi) => {
      ...
      b.columns.forEach((c, ci) => {
        ...
        c.cards.forEach((k, ki) => {
          ik.run(k.id, c.id, k.title, k.notes, k.due, k.enteredColumnAt, ki);
